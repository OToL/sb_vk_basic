...
static char const * const REQUIRED_PHYSICAL_DEVICE_EXTENSIONS [] =
{
...
public:
    ...
    b8 render(GLFWwindow * wnd_hdl)
    {
        ...
        ui32 img_index = 0;
        auto const acq_res = vkAcquireNextImageKHR(m_device, m_swap_chain, wstd::numeric_limits<ui64>::max(), m_img_available_sems[m_curr_frame], VK_NULL_HANDLE, &img_index);
...
private:
    ...
    void recreateSwapChain(GLFWwindow * wnd_hdl)
    {
        ...
        destroyFrameBuffers();
        vkFreeCommandBuffers(m_device, m_cmd_pool, (ui32)m_cmd_buffers.size(), m_cmd_buffers.data()); // We have to free command buffers manually because we are not destroying the associated command pool which is owning them
        destroyCommandBuffers();
        destroyGraphicsPipeline();
        destroyRenderPass();
        destroySwapChainImageViews();
        ...
        if (0 != family_cnt)
        {
            ...
            for (auto const & queue_props : props)
            {
                if (queue_props.queueCount > 0) 
                {
                    ...
                    vkGetPhysicalDeviceSurfaceSupportKHR(device, numericCast<ui32>(curr_idx), surface, &support_present);
    ...
    static b8 isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR surface)
    {
        ...
        return findQueueFamilies(device, surface).isValid() &&
            ...
            checkDeviceSwapChainSupport(device, surface) &&
    ...
    VkExtent2D chooseSwapChainImageExtent(VkSurfaceCapabilitiesKHR const & caps, GLFWwindow * wnd_hdl)
    {
        ...
        else
        {
            ...
            return {wstd::max(caps.minImageExtent.width, wstd::min(caps.maxImageExtent.width, numericCast<ui32>(width))),
                wstd::max(caps.minImageExtent.height, wstd::min(caps.maxImageExtent.height, numericCast<ui32>(height)))};
        }
    }
    ...
    b8 createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags mem_pros, VkBuffer &buffer, VkDeviceMemory &buffer_mem)
    {
        VkBufferCreateInfo info = {};
        info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        info.size = size;
        info.usage = usage;
        info.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // This is the queue family ownership ... this buffer will only be used with the graphics queue
    ...
    b8 createUniformBuffers()
    {
        ...
        {
            createBuffer(sizeof(UniforBufferObject), 
    ...
    b8 createImage(ui32 tex_width, ui32 tex_height, VkFormat format, 
    ...
    VkFormat findSupportedFormat(wstd::span<VkFormat const> formats, VkImageTiling tiling, VkFormatFeatureFlags features)
    {
    ...
    VkFormat findDepthBufferFormat()
    {
        ...
        return findSupportedFormat(depth_formats, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
    }
    ...
    b8 createDepthBuffer()
    {
        ...
        if (!createImage(m_swap_chain_img_ext.width, m_swap_chain_img_ext.height, fmt, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_depth_img, m_depth_img_mem))
        {
    ...
    b8 createTextureImage()
    {
        ...
        if (!createImage(numericCast<ui32>(tex_width), numericCast<ui32>(tex_height), 
            VK_FORMAT_R8G8B8A8_UNORM, 
            VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 
            m_texture_img, m_texture_mem))
        {
            sbLogE("Failed to create texture image");
            return false;
        }

        transitionImageLayout(m_texture_img, VK_FORMAT_R8G8B8A8_UNORM,
    ...
    void transitionImageLayout(VkImage img, VkFormat fmt,
    ...
    b8 createSwapChain(GLFWwindow * wnd_hdl)
    {
        ...
        ui32 const queue_indices [] =
        {
            numericCast<ui32>(queue_family.m_graphics_idx),
        ...
        create_info.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; // We will render directly into the swap chain image
        ...
        // We don't care if the pixels occluded by other operating system elements (windo on top of ours) are rendered/presented or not
        // Because we don't want to read them back
    ...
    b8 createLogicalDevice()
    {
        ...
        // The best approach would be to findout unique queue family IDs to create as less queues as possible e.g. using a set
    ...
    {
        auto create_fn = (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(m_instance, "vkCreateDebugUtilsMessengerEXT");
    ...
    void removeDebugCallback()
    {
        ...
        {
            auto const destroy_fn = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(m_instance, "vkDestroyDebugUtilsMessengerEXT");
    ...
    b8 createInstance()
    {
        ...
        if (m_validation_enabled)
        {
                ...
                (0 != validation_layer_cnt))
            {
                ...
                {
                    auto const layer_name_iter = wstd::find_if(wstd::begin(DEFAULT_VALIDATION_LAYERS), wstd::end(DEFAULT_VALIDATION_LAYERS), [&props] (char const *layer_name) {
